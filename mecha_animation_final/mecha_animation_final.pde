...
public int sequence, numSequence; // number of types of scenes

float rX, rY, rW, rH, rX1, rY1, rW1, rH1; // animation specific values

int lilFrame, lilFrame2;  // frame counters
...
void setup() {
  ...
  // initialize and start tracking the kinect
...
//  output2 = pg[13].get((int)maskLoc2, 0, maskHeight, height);
  ...
  //image(output, 0, (int)maskLoc1);
  //image(output2, (int)maskLoc2, 0);
}

void getKinectHands() {
  ArrayList<KSkeleton> skeletons = kinect.getSkeletonColorMap();
...
void drawLines(PGraphics pg) {
  ...
  if (lilFrame < heightFrame) { // rising white
    ...
    rY = rY - rSpeed;
  } else if (lilFrame >= heightFrame*3.5 && lilFrame < heightFrame*3.75) {
  } // pause 
